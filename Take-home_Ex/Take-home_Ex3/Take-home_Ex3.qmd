...
```{r}
pacman::p_load(tidyverse,tsibble,feasts,fable,seasonal,tidymodels,timetk,modeltime,forecast,ggplot2)
```
...
## 4.3 facet_wrap() for effective comparison of multiple line graph
...
ts_data %>%
  fill_gaps() %>%  # Convert implicit gaps to explicit missing values
  mutate(mean_temperature_c = na.interp(mean_temperature_c)) %>%  # Interpolate missing values
  filter(station == "Paya Lebar") %>%
  model(STL(mean_temperature_c ~ season(window = "periodic"))) %>%
  components() %>%
  autoplot()
...
ts_data %>%
  ...
  model(
    classical_decomposition(
      mean_temperature_c, type = "additive")) %>%
  components() %>%
  autoplot()
```

# 7.0 Visual Forecasting
...
## 7.2 Fitting Forecasting Model

### 7.2.1 Fitting ETS Model
...
### 7.2.7 Forecasting future values
...
### 7.2.8 **Visualising AutoETS model with ggplot2**
